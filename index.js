...
let newName = "Vincent Van Gogh";
artists[8].name = newName;
...
function get20s(artists){
  ...
  for (let i in artists){
    if (parseInt(artists[i].years) >= 1900){
      centuryArtist.push(artists[i].name);
    }
  }
  return centuryArtist;
}
...
function addArtist(artists){
    let newObject = {
      ...
      "bio": "Slap kitten brother with paw need to chase tail. Shed everywhere shed everywhere stretching attack your ankles chase the red dot, hairball run catnip eat the grass sniff good morning sunshine so pose purrfectly to show my beauty. Behind the couch whatever and kitty pounce."
    }
